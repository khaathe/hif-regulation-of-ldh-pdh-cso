...
\begin{figure}
    ...
    \caption{ 
        ...
        (\DIFaddbeginFL \DIFaddFL{$\gamma_{O_2 \to u} = 10.0$, $\gamma_{u \to v} = 3.61$, $\gamma_{u \to w} = 6.97$, $\gamma_{w \to z} = 0.14$)
        (}\DIFaddendFL B) Result with inhibition of the oxygen-dependant degradation of \acrshort{hif}.
        (\DIFaddbeginFL \DIFaddFL{$\gamma_{O_2 \to u} = 1.0$, $\gamma_{u \to v} = 3.61$, $\gamma_{u \to w} = 6.97$, $\gamma_{w \to z} = 0.14$)
        (}\DIFaddendFL C) Result with over-degradation of \acrshort{hif} by oxygen.
        (\DIFaddbeginFL \DIFaddFL{$\gamma_{O_2 \to u} = 40.0$, $\gamma_{u \to v} = 3.61$, $\gamma_{u \to w} = 6.97$, $\gamma_{w \to z} = 0.14$)
        (}\DIFaddendFL D) Result with loss of upregulation of \acrshort{ldh} by \acrshort{hif}.
        \DIFaddbeginFL \DIFaddFL{($\gamma_{O_2 \to u} = 10.0$, $\gamma_{u \to v} = 3.0$, $\gamma_{u \to w} = 6.97$, $\gamma_{w \to z} = 0.14$)
    }\DIFaddendFL }
...
\begin{figure}
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{img/environmental-condition/tumour_current_phase}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{img/tumour_current_phase}
    \DIFaddendFL \caption{ 
...
\begin{figure}
    ...
    \caption{
        Picture of a spheroid grown for 30 days \DIFaddbeginFL \DIFaddFL{in an experiment run in the laboratory}\DIFaddendFL .
...
It seems that varying the concentration of oxygen from normoxia to hypoxia, and reversing this process, every 6 hours \DIFdelbegin \DIFdel{doesn't }\DIFdelend \DIFaddbegin \DIFadd{does not }\DIFaddend affect the diameter of the tumour at the end of the simulation.
...
\begin{figure}
    ...
    \caption{ 
        ...
        The graph shows the results \DIFdelbeginFL \DIFdelFL{at different times }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{after 7 days of growth }\DIFaddendFL for different conditions.
        In oscillating conditions, oxygen concentration is slowly decreased from normoxia to hypoxia during 6h, then oxygen is increased to normoxia at the same rate.
        This process is repeated until the end of the simulation.
...
\DIFaddbegin \DIFadd{Here}\DIFaddend , tumour growth is initiated in normoxia \DIFaddbegin \DIFadd{(results not shown)}\DIFaddend . 
...
Results are similar to normoxic conditions with no genetic deregulations (\DIFaddbegin \DIFadd{presented in }\DIFaddend figure \ref{fig:tumour-state-environment}). 
...
This suggests that the Warburg Effect \DIFdelbegin \DIFdel{isn't }\DIFdelend \DIFaddbegin \DIFadd{is not }\DIFaddend irreversible with a reduced \acrshort{hif} degradation rate by oxygen alone. 
...
\begin{figure}
    ...
    \caption{
        Plot of level of \acrshort{pdh} against the level of \acrshort{ldh} coloured by the extracellular oxygen concentration.
        The graph shows the results \DIFdelbeginFL \DIFdelFL{at different times }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{after 7 days of growth }\DIFaddendFL with three different genetic perturbations: reduced oxygen induced degradation  of \acrshort{hif} ($\gamma O_2 \to \acrshort{hif} = 8.0 $), lower use of glycolysis in hypoxic conditions ( $\gamma \acrshort{hif} \to \acrshort{ldh} = 2.0 $) and lower effect of hypoxia on oxygen consumption ( $\gamma \acrshort{pdk} \to \acrshort{pdh} = 0.7 $).
        Tumour growth was initiated in normoxia.
    }
    \label{fig:ldh-pdh-genetic}
\end{figure}

As expected, reducing the increase in \acrshort{ldh} levels due to \acrshort{hif} response \DIFdelbegin \DIFdel{doesn't }\DIFdelend \DIFaddbegin \DIFadd{does not }\DIFaddend induce a high acidification rate in normoxia but affects the maximum acid production rate and level of \acrshort{ldh}.
...
Since \acrshort{pdh} \DIFdelbegin \DIFdel{isn't }\DIFdelend \DIFaddbegin \DIFadd{is not }\DIFaddend effectively regulated by \acrshort{hif}, the cell tends to stay in an oxidative state and rely less on glycolysis.
...
\acrshort{pdh} levels \DIFdelbegin \DIFdel{don't }\DIFdelend \DIFaddbegin \DIFadd{do not }\DIFaddend fall far below 0.75 even after 7 days of growth compared to others conditions, indicating that cells can only adopt an oxidative or hybrid state.
