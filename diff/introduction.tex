...
\DIFadd{Different isoforms of both }\acrshort{ldh} \DIFadd{and }\acrshort{pdh} \DIFadd{enzymes exist.
...
For example, }\acrshort{ldh}\DIFadd{-A is expressed in the skeletal muscle }\acrshort{ldh}\DIFadd{-B in heart \mbox{%DIFAUXCMD
\cite{Marin-Hernandez2009}}\hspace{0pt}%DIFAUXCMD
. 
}\acrshort{ldh}\DIFadd{-A is also the isoform comonly upregulated in cancer \mbox{%DIFAUXCMD
\cite{Granchi2010}}\hspace{0pt}%DIFAUXCMD
.
...
to lactate}\DIFdelend \DIFaddbegin \acrshort{ldh}\DIFaddend , resulting in higher lactate secretion which acidifies the microenvironment.
...
\cite{Li2020}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend Not only hypoxia will increase the use of glycolysis by the cell, but it will
...
The Warburg effect results in an increased production of lactic acid by the tumour by metabolizing glucose, even in normoxia \cite{Smallbone2007, Leung2017, Robey2005, Jia2019, Courtnay2015}\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < %\DIFdel{Gibin: It is a nice little introduction. We may shorten it if needed}\DIFdelend .
