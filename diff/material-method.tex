...
\begin{table}
    ...
    \caption{
        ...
        The \DIFdelbeginFL \DIFdelFL{absence of unit means that the parameter is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{symbol "-" stands for }\DIFaddendFL dimensionless.
    }
...
In normoxia, \DIFdelbegin \DIFdel{cells mostly use aerobic pathways (glycolysis + }\DIFdelend \DIFaddbegin \DIFadd{glycolysis transforms glucose to pyruvate, then pyruvate is converted to 
...
Furthermore, }\DIFdelend \DIFaddbegin \DIFadd{using oxygen, which constitutes the aerobic pathway \mbox{%DIFAUXCMD
\cite{Zimmerman2011}}\hspace{0pt}%DIFAUXCMD
.
Since the conversion of pyruvate to AcetylCoA is catalyzed by the PDH enzyme, its availability bounds the use of }\acrshort{tca} \DIFadd{and should be reflected in the consumption of oxygen.
...
\DIFadd{In equations }\ref*{formula:po} \DIFadd{and }\ref*{formula:pg}\DIFadd{, we }\DIFaddend define $p_O$ and $p_G$, two terms to \DIFdelbegin \DIFdel{adjust the consumption }\DIFdelend \DIFaddbegin \DIFadd{describe the impact of }\acrshort{ldh} \DIFadd{and }\acrshort{pdh} \DIFadd{on glucose and oxygen consumptions using a sigmoid function based on the logistic funtion.
$p_O$ and $p_G$ will adjust the consumptions }\DIFaddend rates of oxygen and glucose \DIFdelbegin \DIFdel{according to their respective key enzyme level in the form:
}\begin{align} 
...
Here, \DIFdelbegin \DIFdel{$p_{X_{MAX}}$ and $p_{X_{MIN}}$ are both the maximal }\DIFdelend \DIFaddbegin \DIFadd{$\phi_O$ and $\phi_G$ are the maximal values for $p_O$ and $p_G$. 
$\psi_O$ }\DIFaddend and \DIFdelbegin \DIFdel{minimal value of $p_X$.
$X$ is }\DIFdelend \DIFaddbegin \DIFadd{$\psi_G$ are the minimal values for $p_O$ and $p_G$.
...
\cite{Robertson-Tessi2015} }\hspace{0pt}%DIFAUXCMD
(a brief description of the complete model is available in supplementary material)}\DIFaddend . \\
...
Therefore, we do not directly model aerobic and anaerobic pathways but rather we compute the theoretical level of }\acrshort{atp} \DIFadd{generated by both processes (equation \ref*{ode:da}).
...
\DIFdelbegin \DIFdel{Quantities consumed and produced by one cell are modelled using the ODE:
}
\DIFdel{with X is the molecule consumed or produced by the cell. The four variables described are oxygen (O),
glucose (G), ATP (ATP), and protons (}%DIFDELCMD < \protons%%%
\DIFdel{). }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Quantity of }\acrshort{atp} \DIFadd{produced by the cell is modelled by an ODE, and }\DIFaddend extracellular quantities of \DIFaddbegin \DIFadd{the }\DIFaddend three molecules oxygen (O), glucose (G) and protons (\protons) are described \DIFdelbegin \DIFdel{in the model by:
}
\DIFdelend \DIFaddbegin \DIFadd{by PDEs in the following equations:
}\begin{align}
    \DIFadd{\label{ode:da}
    \frac{dA}{dt} }&\DIFadd{= f_A }\\
    \DIFadd{\label{pde:do}
    \frac{\partial O}{\partial t} }&\DIFadd{= D_O \nabla^2 O - \sum_{i=1}^{N} f_O^i }\\
    \DIFadd{\label{pde:dg}
    \frac{\partial G}{\partial t} }&\DIFadd{= D_G \nabla^2 G - \sum_{i=1}^{N} f_G^i }\\
    \DIFadd{\label{pde:dh}
    \frac{\partial H^+}{\partial t} }&\DIFadd{= D_{H^+} \nabla^2 H^+ + \sum_{i=1}^{N} f_{H^+}^i 
}\end{align}\DIFaddend 
\DIFdelbegin \DIFdel{$X$ is the diffusible molecule, $D_X$ is }\DIFdelend \DIFaddbegin \DIFadd{$D_O$, $D_G$ and $D_{H^+}$ are }\DIFaddend the diffusion coefficient for \DIFdelbegin \DIFdel{molecule X, $f_X^i$ describe }\DIFdelend \DIFaddbegin \DIFadd{each molecules. $N$ is the number of cells in the domain with $f_O^i$, $f_G^i$ and $f_{H^+}^i$ }\DIFaddend the impact of \DIFdelbegin \DIFdel{each cell $i$ on the }\DIFdelend \DIFaddbegin \DIFadd{the cell i on the local }\DIFaddend extracellular concentration. \\
\DIFaddbegin \DIFadd{Initial values for oxygen in equation }\ref*{pde:do} \DIFadd{are $O(x,y,0) = 0.056$  mmol/L in normoxia and 0.012 mmol/L in hypoxia.
Initial value for glucose in equation }\ref*{pde:dg} \DIFadd{is $G(x,y,0) = 5.0$  mmol/L.
Initial value for protons in equation }\ref*{pde:dh} \DIFadd{is $H^+(x,y,0) = 3.98.10^{-5}$  mmol/L (pH 7.4).
Let $x_0$ and $y_0$ the lower boundary of the domain in $x$ and $y$, $x_L$ and $y_L$ the upper boundary in $x$ and $y$.
Boundary values for oxygen in equation }\ref*{pde:do} \DIFadd{are $O(x_0,y,t) = O(x_L,y,t) = O(x,y_0,t) = O(x,y_L,t) =$ 0.056 mmol/L in normoxia and 0.012 mmol/L in hypoxia.
Boundary values for glucose in equation }\ref*{pde:dg} \DIFadd{are $G(x_0,y,t) = G(x_L,y,t) = G(x,y_0,t) = G(x,y_L,t) = 5.0$ mmol/L.
Boundary values for $H^+$ in equation }\ref*{pde:dh} \DIFadd{are $H^+(x_0,y,t) = H^+(x_L,y,t) = H^+(x,y_0,t) = H^+(x,y_L,t) = 3.98.10^{-5}$ mmol/L (pH 7.4).
}\DIFaddend Parameters used in those functions are summarised in \DIFdelbegin \DIFdel{table }\DIFdelend \DIFaddbegin \DIFadd{Table }\DIFaddend \ref{tab:param-metabolism}.
...
\begin{table}
    ...
    \caption{
        ...
        \DIFdelbeginFL \DIFdelFL{Dimensionless unit are indicated with -.}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The symbol "-" stands for dimensionless.
    }\DIFaddendFL }
...
\DIFaddbegin \DIFadd{Most aspect of the model are handled by the physicell software }\cite*{Ghaffarizadeh2018}\DIFadd{, this includes : cell division and progression through the cell cycle, cell adhesion and repulsion, substrate diffusion, cell exchanges with the environment (secretion and consumption).
...
The governing ODEs (equation \ref{eq:hif} - \DIFaddbegin \DIFadd{\ref{eq:pdh} and \ref{ode:da}}\DIFaddend ) and PDEs (equation \DIFaddbegin \DIFadd{\ref{pde:do} - \ref{pde:dh}}\DIFaddend ) are run at each timestep to compute cell nutrient consumption, energy and acidity production for that period. 
...
\DIFadd{To simulate the cell entry into quiescence, we created a phase G0 with a reversible link to the }\DIFaddend G1 \DIFdelbegin \DIFdel{, 8h in S, 4h in G2 and 1h in M, for a total of 18h to complete a cell cycle \mbox{%DIFAUXCMD
\cite{Cooper2000}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{phase of the cell cycle.
...
Once the level of }\acrshort{atp} \DIFadd{rises again, we revert the transition rates values to allow the cell to leave the G0 phase and divide anew.
The cell can only transit to the G0 phase from the G1 phase, thus it will complete its cycle once it leaved the G1 phase and will divide even if }\acrshort{atp} \DIFadd{levels fall while the division process is ongoing}\DIFaddend .
