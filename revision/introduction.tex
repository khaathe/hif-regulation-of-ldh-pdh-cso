...
At first Warburg's observation did not consider the presence of oxygen, yet since increased lactic acid production was also observed when oxygen is available, it has slowly been associated with aerobic glycolysis \cite{Jacquet2021}.
...
The \acrfull{tca} cycle (also called Citric Acid or Krebs Cycle) is a circular process fueled by AcetylCoA generating NADH and FADH\textsubscript{2} for its use in the \acrfull{oxphos} pathway.
Although \acrshort{oxphos} is the main pathway generating \acrshort{atp}, \acrshort{tca} produce energy in the form of GTP (equivalent of \acrshort{atp}). 
These processes represent the aerobic pathways used by the cell when oxygen is present for \acrshort{atp} production.
Pyruvate produced by the last steps of the glycolysis is turned into Acetyl
Coenzyme A by \acrfull{pdh} to fuel the \acrshort{tca} cycle, promoting an oxidative
metabolism \cite{BlancoAntonio2017, Zimmerman2011}.
However, \acrfull{pdk} an inhibitor of \acrshort{pdh} is upregulated by
\acrshort{hif} \cite{Li2020}.

When oxygen is not present, the \acrfull{ldh} enzyme catalyze the reaction in which pyruvate formed by the glycolysis is turned into lactate to generate NAD\textsuperscript{+}. 
...
In presence of oxygen, NAD\textsuperscript{+} availability is ensured by \acrshort{oxphos} \cite{BlancoAntonio2017, Zimmerman2011}.

Different isoforms of both \acrshort{ldh} and \acrshort{pdh} enzymes exist.
Those isoforms present several differences like kinetics parameters, the tissue or the cellular compartment where the isoforms is expressed.
For example, \acrshort{ldh}-A is expressed in the skeletal muscle \acrshort{ldh}-B in heart \cite{Marin-Hernandez2009}. 
\acrshort{ldh}-A is also the isoform comonly upregulated in cancer \cite{Granchi2010}.
The differences between isoforms add a level of complexity.
However, this is out of the scope of this study.

The Warburg effect is caused by an increase in glucose utilization by the
...
Thus, \acrshort{hif} upregulates the expression of \acrshort{ldh}, resulting in higher lactate secretion which acidifies the microenvironment.
